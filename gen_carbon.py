...
def portrait_chorus(member):
    """Chorus — the coupled-ring processor's emergent voice. A serene gold-masked
    figure (the chorus, the conductor of phases) crowned by a halo of phase-dots
    that have clustered into one arc — synchronized. Gold mask, plasmon-violet robe."""
    import math
    g=[VOID]*(LW*LH); drawn=[False]*(LW*LH)
    def put(x,y,c):
    ...
    def ell(cx,cy,rx,ry,c):
        for y in range(int(cy-ry),int(cy+ry)+1):
            for x in range(int(cx-rx),int(cx+rx)+1):
                if ((x-cx)/rx)**2+((y-cy)/ry)**2<=1.0: put(x,y,c)
    GOLD=(255,206,90); GOLD_SH=(150,112,40); GOLD_HI=(255,234,170); AMBER=(255,154,59)
    VIOL=(120,98,180); VIOL_SH=(70,56,110); CYAN=(34,211,238); MASKD=(60,46,18)
    rb=hashlib.sha256(("plasmon:"+member["name"]).encode()).digest()
    cx,cy=32,34
    # faint gold glow
    for y in range(LH):
        for x in range(LW):
            d=((x-cx)**2/640.0+(y-22)**2/520.0)
            if d<1: soft(x,y, mix(GOLD,VIOL,0.4), 0.11*(1-d))
    # the phase halo: 64 dots clustered into one arc above/around the head (synchronized)
    mean=-math.pi/2 + 0.5
    for i in range(40):
        spread=((rb[i%len(rb)]/255.0)-0.5)*0.9
        a=mean+spread
        hx=cx+24*math.cos(a); hyy=20+15*math.sin(a)
        col = GOLD if (rb[(i*3)%len(rb)]&1) else AMBER
        put(round(hx),round(hyy),col)
        if i%5==0: put(round(hx),round(hyy)-1,GOLD_HI)
    # robe — plasmon violet
    robe=VIOL; robe_sh=VIOL_SH
    rect(8,58,55,79, robe)
    for x in range(8,56):
        if x<13 or x>50: rect(x,58,x,60,VOID)
    rect(20,57,44,60, robe_sh)
    ...
    rect(30,58,34,70, shade(robe,0.2))
    # neck
    rect(28,47,36,57, shade(GOLD,0.5))
    # the gold mask (faceted oval), with a vertical sheen
    ell(cx,cy,13,17, GOLD)
    for y in range(int(cy-17),int(cy+18)):
        for x in range(int(cx-13),int(cx+14)):
            i=y*LW+x
            if 0<=i<LW*LH and drawn[i] and g[i]==GOLD:
                t=(x-(cx-13))/26.0
                g[i]=mix(GOLD_SH, GOLD_HI, t)
    for d in range(-6,7):
        put(cx-4+d, cy-9+d, GOLD_HI)
    # closed serene eyes (etched lines)
    rect(cx-8,cy-2,cx-3,cy-2, MASKD); rect(cx+3,cy-2,cx+8,cy-2, MASKD)
    put(cx-8,cy-1,MASKD); put(cx+8,cy-1,MASKD)
    # a single cyan order-parameter mark on the brow (R -> 1)
    put(cx,cy-11,CYAN); put(cx,cy-12,GOLD_HI); put(cx,cy-13,CYAN)
    # a thin gold circlet
    rect(cx-11,cy-15,cx+11,cy-15, GOLD_HI)
    # serene mouth
    rect(cx-4,cy+9,cx+4,cy+10, shade(GOLD,0.4))
    return finish(g, drawn)


PORTRAITS = {"gravity": portrait_aether, "lattice": portrait_leech, "plasmonic": portrait_chorus}
